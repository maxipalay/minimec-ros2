/// @file
/// @brief Generates a reference trajectory.
///
/// PUBLISHERS:
///     /plan (nav_msgs::msg::Path): output reference trajectory/plan/path
///     /points (visualization_msgs::msg::MarkerArray): reference points
/// SERVICES:
///     /generate_spline_plan (minimec_msgs::srv::SplinePlanRequest): spline plan request
///     /generate_circular_plan (minimec_msgs::srv::CircularPlanRequest): circular plan request
...
public:
  ...
  : Node("path_generator")
  {
    ...
    spline_path_service_ =
      ...
      std::bind(
        &PathGenerator::splinePlanCallback, this, std::placeholders::_1,
        std::placeholders::_2));
    ...
    circular_path_service_ =
      ...
      std::bind(
        &PathGenerator::circularPlanCallback, this, std::placeholders::_1,
        std::placeholders::_2));

    // qos for plan & marker array publishing
...
private:
  void circularPlanCallback(
    const std::shared_ptr<minimec_msgs::srv::CircularPlanRequest::Request> request,
  ...
  void splinePlanCallback(
    const std::shared_ptr<minimec_msgs::srv::SplinePlanRequest::Request> request,
    std::shared_ptr<minimec_msgs::srv::SplinePlanRequest::Response> response)
  {
    ...
    for (size_t i = 1; i < 100; i++) {
      std::vector<tsReal> result1 =
        spline.eval(
        static_cast<double>(i - 1) / static_cast<double>(100 - 1) *
        static_cast<double>(domain_max)).result();
      std::vector<tsReal> result2 = spline.eval(
        static_cast<double>(i) / static_cast<double>(100 - 1) *
        static_cast<double>(domain_max)).result();
    ...
    for (size_t i = 0; i < steps; i++) {
      ...
      if (request->heading_mode == "fixed") {
        ...
        auto angle = std::atan2(
          request->heading_point.y - pose.pose.position.y,
          request->heading_point.x - pose.pose.position.x);
        rot.setRPY(0.0, 0.0, angle);
      }
      pose.pose.orientation.x = rot.getX();
      pose.pose.orientation.y = rot.getY();
